...
module.exports.record = data => {
    if (!history[data.source]){
        ...
        if (!location) return;
        history[data.source] = { values: [], domain : data.source, name : location.name, icon:location.icon, cdn:!!location.cdn};
    }
    ...
    var max = Object.keys(history).map(x => history[x].average).reduce((a,b) => Math.max(a,b));
    value.percent= 0;
    if (max > 0){
        Object.keys(history).forEach(x => history[x].percent = 100 * history[x].average / max);
    }
